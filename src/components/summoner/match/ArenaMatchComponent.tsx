...
import {
	...
	calculateDatePlayed,
	calculateKDA,
	calculateKdaColor, chunkArenaParticipants, fallbackSummonerSpellIconUrl,
	findPlayer, getArenaPlacementForParticipant, getItemIconUrlByItemId, getMapUrlByMapId, getSummonerSpellIconUrl,
		...
		<div>
			<Base className={`match ${className} ${player.win ? 'player-won' : 'player-lost'}`}>
				...
				<div className="arena-player-stats">
					...
					<div className="arena-items">
						{itemFields.map((key, index) => (
							<div key={key ?? 'unknownItemField' + index} className="arena-item-container">
								{player[key] !== 0 ? (
									<img
				...
				<div className="arena-teams">
					{chunkArenaParticipants(participants, 4).map((participantArray, index) => (
						<div key={`participant-column-${index}`} className={"arena-team"}>
